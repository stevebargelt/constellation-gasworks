...
> **SMTP and dashboard credentials are provisioned automatically by OpenTofu** — no additional GitHub secrets are required for them:
> - **SMTP**: host (`smtp.resend.com`), port (`465`), username (`resend`), and sender name (`Constellation`) are hardcoded in `keyvault.tf`. The SMTP password (`CONSTELLATION-SMTP-PASS`) is set to the value of `TF_VAR_RESEND_API_KEY`.
> - **Dashboard**: username is hardcoded as `supabase`; password is a random 32-character string generated by OpenTofu on first `apply` and stored in Key Vault as `CONSTELLATION-DASHBOARD-PASSWORD`. Retrieve it after apply:
>   ```bash
...
Fill in `secrets.tfvars` with values from Step 1 + your Resend and New Relic keys:
```hcl
